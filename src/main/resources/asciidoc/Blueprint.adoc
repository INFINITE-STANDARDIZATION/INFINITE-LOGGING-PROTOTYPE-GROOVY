...
IMPORTANT: Assurance of compliance to these principles must define the foundation of Test Strategy of the solution.
...
|MapEntryExpression
|No transformation
|Interim expression which does not itself evaluate to single object +
Logging is injected into its parent and child nodes (key/values and map).

|ArgumentListExpression
|No transformation
|Interim expression which does not itself evaluate to single object +
Logging is injected into its parent and child nodes.

|DeclarationExpression
|*Not* wrapped into MethodCallExpression; transformed into ListOfExpressionsExpression having the below expressions:

<1> Injected logging code (MethodCallExpression)
<2> Declaration expression with *transformed* right expression
<3> Injected logging code (MethodCallExpression)
|* Left expression of declaration is never transformed
...
* *Q: Why do we inject code only for DeclarationExpression?*
** A: Declaration expression does not evaluate itself to object and can't be replaced with MethodCall expression. +
Additionally VariableScopeVisitor must be able to declare the variable after BlackBox transformation into the same branch of variable scopes. +
...
|BinaryExpression
|BinaryExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
BinaryExpression:rightExpression is transformed. +
BinaryExpression:leftExpression is conditionally transformed.
|* Right expression is always transformed
* Left expression is transformed only when operation is *not* "Assignment" ("=").


|BitwiseNegationExpression
|BitwiseNegationExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
BitwiseNegationExpression:expression is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|NotExpression
|NotExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
NotExpression:expression is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|BooleanExpression
|BooleanExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
BooleanExpression:expression is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|CastExpression
|CastExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
ClassExpression:expression is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|ConstructorCallExpression
|ConstructorCallExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
ConstructorCallExpression:arguments is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|MethodPointerExpression
|MethodPointerExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
MethodPointerExpression:expression and MethodPointerExpression:methodName is transformed
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|AttributeExpression
|AttributeExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
AttributeExpression:objectExpression and PropertyExpression:property is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|PropertyExpression
|PropertyExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
PropertyExpression:objectExpression and PropertyExpression:property is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|RangeExpression
|RangeExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
RangeExpression:from and RangeExpression:to is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|SpreadExpression
|SpreadExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
SpreadExpression:expression is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|SpreadMapExpression
|SpreadMapExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
SpreadExpression:expression is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|StaticMethodCallExpression
|StaticMethodCallExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
StaticMethodCallExpression:arguments is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|ElvisOperatorExpression
|ElvisOperatorExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
ElvisOperatorExpression:trueExpression and ElvisOperatorExpression:falseExpression is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|TernaryExpression
|TernaryExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
TernaryExpression:booleanExpression, TernaryExpression:trueExpression, TernaryExpression:falseExpression is transformed.
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|UnaryMinusExpression
|UnaryMinusExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
UnaryMinusExpression:expression is transformed
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|UnaryPlusExpression
|UnaryPlusExpression is wrapped into MethodCallExpression ("expressionEvaluation" method). +
UnaryPlusExpression:expression is transformed
|Whole expression is replaced with new instance outside "visit" method method (due to GROOVY-8834)

|MethodCallExpression
|
|Transformed in its corresponding Visit method.
